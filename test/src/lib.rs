...
use lean_ffi::object::{
  ExternalClass, LeanArray, LeanBool, LeanByteArray, LeanCtor, LeanExcept,
  LeanExternal, LeanIOResult, LeanList, LeanNat, LeanObject, LeanOption,
  LeanProd, LeanString,
};
...
pub extern "C" fn rs_roundtrip_list_nat(list_ptr: LeanList) -> LeanList {
  ...
  let items: Vec<LeanObject> = nats.iter().map(|n| build_nat(n)).collect();
