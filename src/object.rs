...
///
/// The [`lean_ctor!`](crate::lean_ctor) and [`lean_inductive!`](crate::lean_inductive)
/// macros compute these byte offsets automatically from declared field counts
/// and attach typed accessor methods, so users don't need to calculate offsets
/// manually. Reading all fields of `Foo` in C and via the generated accessors:
///
/// ```c
...
///
/// ```ignore
/// // Rust — generated by lean_ctor! / lean_inductive! (no manual offsets)
/// let name  = foo.get_obj(0);      // object field
/// let size  = foo.get_usize(0);    // first USize field
/// let count = foo.get_num_64(0);   // first 8-byte scalar
/// let flag  = foo.get_num_8(0);    // first 1-byte scalar
/// ```
#[repr(transparent)]
